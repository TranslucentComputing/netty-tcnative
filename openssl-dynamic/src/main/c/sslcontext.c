...
static void ssl_context_cleanup(void *data)
{
...
TCN_IMPLEMENT_CALL(jlong, SSLContext, make)(TCN_STDARGS, jint protocol, jint mode)
{
    ...
    if ((c = OPENSSL_malloc(sizeof(tcn_ssl_ctxt_t))) == NULL) {
        tcn_Throw(e, "Failed to OPENSSL_malloc tcn_ssl_ctxt_t");
        goto cleanup;
    }
    // TODO: Should we not need to memset but just explicit set everything in the struct ?
    memset(c, 0, sizeof(tcn_ssl_ctxt_t));
...
{
    tcn_ssl_ctxt_t *c = J2P(ctx, tcn_ssl_ctxt_t *);
    jlong rv = tcn_atomic_uint32_get(&c->ticket_keys_new);
    return rv;
}
...
{
    tcn_ssl_ctxt_t *c = J2P(ctx, tcn_ssl_ctxt_t *);
    jlong rv = tcn_atomic_uint32_get(&c->ticket_keys_resume);
    return rv;
}
...
{
    tcn_ssl_ctxt_t *c = J2P(ctx, tcn_ssl_ctxt_t *);
    jlong rv = tcn_atomic_uint32_get(&c->ticket_keys_renew);
    return rv;
}
...
{
    tcn_ssl_ctxt_t *c = J2P(ctx, tcn_ssl_ctxt_t *);
    jlong rv = tcn_atomic_uint32_get(&c->ticket_keys_fail);
    return rv;
}
...
static int current_session_key(tcn_ssl_ctxt_t *c, tcn_ssl_ticket_key_t *key) {
    ...
    tcn_lock_reader_t reader_lock = tcn_lock_reader_acquire(c->ticket_keys_lock);
    if (c->ticket_keys_len > 0) {
        *key = c->ticket_keys[0];
        result = JNI_TRUE;
    }
    tcn_lock_reader_release(reader_lock);
    return result;
}
...
TCN_IMPLEMENT_CALL(void, SSLContext, setSessionTicketKeys0)(TCN_STDARGS, jlong ctx, jbyteArray keys)
{
    ...
    tcn_lock_reader_t writer_lock = tcn_lock_writer_acquire(c->ticket_keys_lock);
    if (c->ticket_keys) {
        OPENSSL_free(c->ticket_keys);
    }
    c->ticket_keys_len = cnt;
    c->ticket_keys = ticket_keys;
    tcn_lock_writer_release(writer_lock);
